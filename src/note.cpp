...
namespace vrv
{
    ...
    std::wstring Note::GetTabFretString(data_NOTATIONTYPE notationType) const
    {
        if (notationType == NOTATIONTYPE_tab_lute_italian)
        {
            ...
            if (fret > 9)
                fretStr.push_back(SMUFL_EBE1_luteItalianFret1);
            switch (fret % 10)
            {
            case 0:
                fretStr.push_back(SMUFL_EBE0_luteItalianFret0);
                break;
            case 1:
                fretStr.push_back(SMUFL_EBE1_luteItalianFret1);
                break;
            case 2:
                fretStr.push_back(SMUFL_EBE2_luteItalianFret2);
                break;
            case 3:
                fretStr.push_back(SMUFL_EBE3_luteItalianFret3);
                break;
            case 4:
                fretStr.push_back(SMUFL_EBE4_luteItalianFret4);
                break;
            case 5:
                fretStr.push_back(SMUFL_EBE5_luteItalianFret5);
                break;
            case 6:
                fretStr.push_back(SMUFL_EBE6_luteItalianFret6);
                break;
            case 7:
                fretStr.push_back(SMUFL_EBE7_luteItalianFret7);
                break;
            case 8:
                fretStr.push_back(SMUFL_EBE8_luteItalianFret8);
                break;
            case 9:
                fretStr.push_back(SMUFL_EBE9_luteItalianFret9);
                break;
            default:
                break;
            }
    ...
    {
        if (!ignoreAccid && !this->IsEnharmonicWith(note))
            return false;
    ...
    Point Note::GetStemUpSE(const Doc *doc, int staffSize, bool isCueSize) const
    {
        ...
        if (isCueSize)
            defaultYShift = doc->GetCueSize(defaultYShift);
    ...
    {
        static std::map<std::string, wchar_t> additionalNoteheadSymbols = {{"noteheadDiamondBlackWide", SMUFL_E0DC_noteheadDiamondBlackWide},
        ...
        {
        case HEADSHAPE_quarter:
            return SMUFL_E0A4_noteheadBlack;
        case HEADSHAPE_half:
            return SMUFL_E0A3_noteheadHalf;
        ...
        case HEADSHAPE_plus:
            return SMUFL_E0AF_noteheadPlusBlack;
        case HEADSHAPE_diamond:
        {
            if (DUR_4 > duration)
                return SMUFL_E0D9_noteheadDiamondHalf;
        ...
        {
            if (DUR_1 >= duration)
                return SMUFL_E102_noteheadSlashWhiteWhole;
            if (DUR_2 == duration)
        ...
        {
            if (DUR_1 == duration)
                return SMUFL_E0B5_noteheadWholeWithX;
            if (DUR_2 == duration)
                return SMUFL_E0B6_noteheadHalfWithX;
        ...
        {
        case NOTEHEADMODIFIER_dblwhole:
            return SMUFL_E0A0_noteheadDoubleWhole;
        default:
            break;
        }

        if (DUR_BR == duration)
            return SMUFL_E0A1_noteheadDoubleWholeSquare;
        if (DUR_1 == duration)
            return SMUFL_E0A2_noteheadWhole;
        if (DUR_2 == duration)
            return SMUFL_E0A3_noteheadHalf;
    ...
    void Note::CalcNoteHeadShiftForSameasNote(Note *stemSameas, data_STEMDIRECTION stemDir)
    {
        ...
        if (abs(this->GetDiatonicPitch() - stemSameas->GetDiatonicPitch()) > 1)
            return;

        Note *noteToShift = this;
        if (stemDir == STEMDIRECTION_up)
        {
            if (this->GetDrawingY() < stemSameas->GetDrawingY())
                noteToShift = stemSameas;
        }
        else
        {
            if (this->GetDrawingY() > stemSameas->GetDrawingY())
    ...
    bool Note::IsDotOverlappingWithFlag(const Doc *doc, const int staffSize, int dotLocShift) const
    {
        ...
        if (dur < DURATION_16)
            flagGlyph = flag->GetFlagGlyph(this->GetDrawingStemDir());
        const int flagHeight = doc->GetGlyphHeight(flagGlyph, staffSize, this->GetDrawingCueSize());

        const int dotMargin = flag->GetDrawingY() - this->GetDrawingY() - flagHeight - this->GetDrawingRadius(doc) / 2 - dotLocShift * doc->GetDrawingUnit(staffSize);
    ...
    bool Note::HandleLedgerLineStemCollision(const Doc *doc, const Staff *staff, const Note *note1, const Note *note2)
    {
        ...
        if (note1->GetDrawingLoc() == note2->GetDrawingLoc())
            return false;
        ...
        if (upperNote->GetDrawingStemDir() != STEMDIRECTION_down)
            return false;
        if (lowerNote->GetDrawingStemDir() != STEMDIRECTION_up)
            return false;
    ...
    int Note::CalcStem(FunctorParams *functorParams)
    {
        ...
        params->m_verticalCenter = staff->GetDrawingY() - params->m_doc->GetDrawingUnit(staffSize) * (staff->m_drawingLines - 1);
    ...
    int Note::CalcDots(FunctorParams *functorParams)
    {
        ...
        if (this->GetDots() > 0)
        {
            ...
            else if ((this->GetDrawingStemDir() == STEMDIRECTION_up) && !this->IsInBeam() && (this->GetDrawingStemLen() < 3) && (this->IsDotOverlappingWithFlag(params->m_doc, staffSize, dotLocShift)))
            {
                // HARDCODED
    ...
    int Note::CalcLedgerLines(FunctorParams *functorParams)
    {
        ...
        if (!this->HasLedgerLines(linesAbove, linesBelow, staff))
            return FUNCTOR_SIBLINGS;
    ...
    int Note::PrepareLayerElementParts(FunctorParams *functorParams)
    {
        ...
        Chord *chord = this->IsChordTone();
        if (currentStem)
            currentFlag = dynamic_cast<Flag *>(currentStem->GetFirst(FLAG));
        ...
        if ((this->GetActualDur() > DUR_4) && !this->IsInBeam() && !this->GetAncestorFTrem() && !this->IsChordTone() && !this->IsMensuralDur() && !this->IsTabGrpNote())
        {
        ...
        {
            assert(currentStem);
            if (currentStem->DeleteChild(currentFlag))
                currentFlag = NULL;
        }
    ...
    int Note::GenerateMIDI(FunctorParams *functorParams)
    {
        ...
        if (this->IsGraceNote())
        {
            ...
            if (graceGrp && (graceGrp->GetGrace() == GRACE_acc))
                accented = true;
        ...
        else
        {
            ...
            if (this->HasTabCourse() && this->GetTabCourse() >= 1)
            {
                ...
                params->m_heldNotes[course - 1].m_stopTime = params->m_totalTime + std::max(defaultHoldTime, this->GetScoreTimeOffset() + this->GetScoreTimeTiedDuration());

...
namespace vrv
{
    ...
    int Measure::GetWidth() const
    {
        ...
        if (m_xAbs2 != VRV_UNSET)
            return (m_xAbs2 - m_xAbs);
    ...
    int Measure::GetDrawingOverflow()
    {
        ...
        if (!adjustXOverflowParams.m_currentWidest)
            return 0;
    ...
    std::vector<Staff *> Measure::GetFirstStaffGrpStaves(ScoreDef *scoreDef)
    {
        ...
        for (auto &staffGrp : staffGrps)
        {
            ...
            if (staffDef)
                staffList.push_back(staffDef->GetN());
        }
    ...
    {
        int repeat = 1;
        double timeDuration = m_measureAligner.GetRightAlignment()->GetTime() * DURATION_4 / DUR_MAX * 60.0 / m_currentTempo * 1000.0 + 0.5;
        std::vector<double>::const_iterator iter;
        for (iter = m_realTimeOffsetMilliseconds.begin(); iter != m_realTimeOffsetMilliseconds.end(); ++iter)
        {
            if ((time >= *iter) && (time <= *iter + timeDuration))
                return repeat;
            repeat++;
        }
        return 0;
    ...
    {
        if ((repeat < 1) || repeat > (int)m_realTimeOffsetMilliseconds.size())
            return 0;
    ...
    data_BARRENDITION Measure::GetDrawingLeftBarLineByStaffN(int staffN) const
    {
        ...
        if (elementIter != m_invisibleStaffBarlines.end())
            return elementIter->second.first;
    ...
    data_BARRENDITION Measure::GetDrawingRightBarLineByStaffN(int staffN) const
    {
        auto elementIter = m_invisibleStaffBarlines.find(staffN);
        if (elementIter != m_invisibleStaffBarlines.end())
            return elementIter->second.second;
    ...
    Measure::BarlineRenditionPair Measure::SelectDrawingBarLines(Measure *previous)
    {
        ...
        if (previousRight == drawingLines.end())
            return defaultValue;

        auto currentLeft = previousRight->second.find(this->GetLeft());
        if (currentLeft == previousRight->second.end())
            return defaultValue;
    ...
    void Measure::SetDrawingBarLines(Measure *previous, int barlineDrawingFlags)
    {
        ...
        else if (!((barlineDrawingFlags & BarlineDrawingFlags::SCORE_DEF_INSERT) || (barlineDrawingFlags & BarlineDrawingFlags::INVISIBLE_MEASURE_CURRENT) || (barlineDrawingFlags & BarlineDrawingFlags::INVISIBLE_MEASURE_PREVIOUS)))
        {
            ...
            else
            {
                ...
                if (right != left)
                {
                    ...
                    if (this->HasInvisibleStaffBarlines())
        ...
        {
            if ((barlineDrawingFlags & BarlineDrawingFlags::INVISIBLE_MEASURE_PREVIOUS) && !(barlineDrawingFlags & BarlineDrawingFlags::INVISIBLE_MEASURE_CURRENT) && !(barlineDrawingFlags & BarlineDrawingFlags::SCORE_DEF_INSERT))
            {
        ...
        Measure *previous, ListOfObjects &currentInvisible, ListOfObjects &previousInvisible, int barlineDrawingFlags)
    {
        ...
        for (const auto object : currentInvisible)
        {
            ...
            if (right == BARRENDITION_NONE)
                right = BARRENDITION_single;
            auto [iter, result] = previous->m_invisibleStaffBarlines.insert({staff->GetN(), {BARRENDITION_NONE, right}});
            if (!result)
                iter->second.second = right;
        }
    ...
    std::vector<std::pair<LayerElement *, LayerElement *>> Measure::GetInternalTieEndpoints()
    {
        ...
        for (Object *object : children)
        {
            ...
            if (!start || (start->GetFirstAncestor(MEASURE) != this))
                continue;
            LayerElement *end = tie->GetEnd();
            if (!end || (end->GetFirstAncestor(MEASURE) != this))
                continue;
    ...
    int Measure::FindSpannedLayerElements(FunctorParams *functorParams) const
    {
        ...
        if (Object::IsPreOrdered(this, params->m_interface->GetStartMeasure()))
            return FUNCTOR_SIBLINGS;

        if (Object::IsPreOrdered(params->m_interface->GetEndMeasure(), this))
            return FUNCTOR_SIBLINGS;
    ...
    int Measure::ApplyPPUFactor(FunctorParams *functorParams)
    {
        ...
        if (m_xAbs2 != VRV_UNSET)
            m_xAbs2 /= params->m_page->GetPPUFactor();
    ...
    int Measure::AlignHorizontally(FunctorParams *functorParams)
    {
        ...
        if (m_leftBarLine.SetAlignment(m_measureAligner.GetLeftBarLineAlignment()))
            params->m_hasMultipleLayer = true;
        if (m_rightBarLine.SetAlignment(m_measureAligner.GetRightBarLineAlignment()))
            params->m_hasMultipleLayer = true;
    ...
    int Measure::AlignHorizontallyEnd(FunctorParams *functorParams)
    {
        ...
        if (params->m_hasMultipleLayer)
            m_hasAlignmentRefWithMultipleLayers = true;
    ...
    int Measure::AdjustLayers(FunctorParams *functorParams)
    {
        ...
        if (!m_hasAlignmentRefWithMultipleLayers)
            return FUNCTOR_SIBLINGS;
    ...
    int Measure::AdjustXPos(FunctorParams *functorParams)
    {
        ...
        int minMeasureWidth = params->m_doc->GetOptions()->m_unit.GetValue() * params->m_doc->GetOptions()->m_measureMinWidth.GetValue();
        ...
        Alignment *fullMeasure2 = dynamic_cast<Alignment *>(m_measureAligner.FindDescendantByComparison(&alignmentComparison, 1));
        ...
        else if (this->FindDescendantByType(MULTIREST) != NULL)
        {
            ...
            if (multiRest->HasWidth())
            {
                ...
                if (minMeasureWidth < fixedWidth)
                    minMeasureWidth = fixedWidth;
            }
            else if (num > 10)
            {
    ...
    int Measure::CastOffSystems(FunctorParams *functorParams)
    {
        ...
        const bool isLeftoverMeasure = ((NULL == nextMeasure) && params->m_doc->GetOptions()->m_breaksNoWidow.GetValue() && (params->m_doc->GetOptions()->m_breaks.GetValue() != BREAKS_encoded));
    ...
    {
        PrepareStaffCurrentTimeSpanningParams *params = vrv_params_cast<PrepareStaffCurrentTimeSpanningParams *>(functorParams);
        assert(params);
    ...
    int Measure::PrepareTimestampsEnd(FunctorParams *functorParams)
    {
        ...
        while (iter != params->m_tstamps.end())
        {
            ...
            else if ((*iter).second.first == 0)
            {
                ...
                if (interface->HasStartAndEnd())
                {
                    auto item = std::find_if(params->m_timeSpanningInterfaces.begin(), params->m_timeSpanningInterfaces.end(),
                                             [interface](
                                                 std::pair<TimeSpanningInterface *, ClassId> pair)
        ...
        for (auto &object : fs)
        {
            ...
            if (harm)
            {
                ...
                auto item = std::find_if(params->m_timeSpanningInterfaces.begin(), params->m_timeSpanningInterfaces.end(),
                                         [f](std::pair<TimeSpanningInterface *, ClassId> pair)
                                         { return (pair.first == f); });
                if (item != params->m_timeSpanningInterfaces.end())
                {
                    // LogDebug("Found it!");
    ...
    int Measure::GenerateTimemap(FunctorParams *functorParams)
    {
        ...
        Measure *measure = vrv_cast<Measure *>(this); // Jonas
        assert(note);                                 // Jonas
    ...
    int Measure::InitMaxMeasureDurationEnd(FunctorParams *functorParams)
    {
        ...
        const double scoreTimeIncrement = m_measureAligner.GetRightAlignment()->GetTime() * params->m_multiRestFactor * DURATION_4 / DUR_MAX;
    ...
    int Measure::CacheHorizontalLayout(FunctorParams *functorParams)
    {
        ...
        if (this->GetLeftBarLine())
            this->GetLeftBarLine()->CacheHorizontalLayout(functorParams);
        if (this->GetRightBarLine())
            this->GetRightBarLine()->CacheHorizontalLayout(functorParams);

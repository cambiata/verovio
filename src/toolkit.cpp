...
namespace vrv
{
    ...
    bool Toolkit::SetFont(const std::string &fontName)
    {
        ...
        if (!ok)
            LogWarning("Font '%s' could not be loaded", fontName.c_str());
        return ok;
    }
    ...
    bool Toolkit::IsZip(const std::string &filename)
    {
        ...
        if (memcmp(data, ZIP_SIGNATURE, 4) == 0)
            return true;
    ...
    bool Toolkit::LoadData(const std::string &data)
    {
        ...
        if (m_options->m_transpose.IsSet() || m_options->m_transposeMdiv.IsSet() || m_options->m_transposeToSoundingPitch.IsSet())
        {
            m_doc.PrepareData();
            m_doc.TransposeDoc();
        }
        ...
        {
            if (input->GetLayoutInformation() == LAYOUT_ENCODED && (breaks == BREAKS_encoded || breaks == BREAKS_line || breaks == BREAKS_smart))
            {
        ...
        case NOTATIONTYPE_mensural_white:
            m_editorToolkit = new EditorToolkitMensural(&m_doc, &m_view);
            break;
        case NOTATIONTYPE_cmn:
    ...
    std::string Toolkit::GetMEI(const std::string &jsonOptions)
    {
        ...
        if (!jsonOptions.empty())
        {
            ...
            {
                if (json.has<jsonxx::Boolean>("scoreBased"))
                    scoreBased = json.get<jsonxx::Boolean>("scoreBased");
                if (json.has<jsonxx::Boolean>("ignoreHeader"))
                    ignoreHeader = json.get<jsonxx::Boolean>("ignoreHeader");
                if (json.has<jsonxx::Boolean>("removeIds"))
                    removeIds = json.get<jsonxx::Boolean>("removeIds");
                if (json.has<jsonxx::Number>("firstPage"))
                    firstPage = json.get<jsonxx::Number>("firstPage");
                if (json.has<jsonxx::Number>("lastPage"))
                    lastPage = json.get<jsonxx::Number>("lastPage");
                if (json.has<jsonxx::Number>("pageNo"))
                {
                    firstPage = json.get<jsonxx::Number>("pageNo");
                    lastPage = firstPage;
                }
                if (json.has<jsonxx::String>("firstMeasure"))
                    firstMeasure = json.get<jsonxx::String>("firstMeasure");
                if (json.has<jsonxx::String>("lastMeasure"))
                    lastMeasure = json.get<jsonxx::String>("lastMeasure");
                if (json.has<jsonxx::String>("mdiv"))
                    mdiv = json.get<jsonxx::String>("mdiv");
            }
        }
        ...
        if (firstPage > 0)
            meioutput.SetFirstPage(firstPage);
        if (lastPage > 0)
            meioutput.SetLastPage(lastPage);
        if (!firstMeasure.empty())
            meioutput.SetFirstMeasure(firstMeasure);
        if (!lastMeasure.empty())
            meioutput.SetLastMeasure(lastMeasure);
        if (!mdiv.empty())
            meioutput.SetMdiv(mdiv);

        std::string output = meioutput.GetOutput();

        if (hadSelection)
            m_doc.ReactivateSelection(false);
    ...
    std::string Toolkit::GetAvailableOptions() const
    {
        ...
        for (auto const &optionGrp : *optionGrps)
        {
            ...
            for (auto const &option : *options)
            {
                ...
                if (optJson && (optJson->GetSource() == JsonSource::FilePath))
                    continue;
            ...
            else if (json.has<jsonxx::Array>(iter->first))
            {
                ...
                for (i = 0; i < (int)values.size(); ++i)
                {
                    if (values.has<jsonxx::String>(i))
                        strValues.push_back(values.get<jsonxx::String>(i));
    ...
    void Toolkit::ResetOptions()
    {
        std::for_each(m_options->GetItems()->begin(), m_options->GetItems()->end(),
                      [](const MapOfStrOptions::value_type &opt)
                      { opt.second->Reset(); });
    ...
    void Toolkit::RedoLayout(const std::string &jsonOptions)
    {
        ...
        if (!jsonOptions.empty())
        {
            ...
            {
                if (json.has<jsonxx::Boolean>("resetCache"))
                    resetCache = json.get<jsonxx::Boolean>("resetCache");
            }
        }
    ...
    std::string Toolkit::RenderToTimemap(const std::string &jsonOptions)
    {
        ...
        if (!jsonOptions.empty())
        {
            ...
            else
            {
                ...
                if (json.has<jsonxx::Boolean>("includeRests"))
                    includeRests = json.get<jsonxx::Boolean>("includeRests");
            }
        }

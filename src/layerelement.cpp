...
namespace vrv
{
    ...
    bool LayerElement::IsGraceNote() const
    {
        ...
        if (this->GetFirstAncestor(GRACEGRP))
            return true;
    ...
    {
        if (!this->Is({CHORD, NOTE, REST, TABGRP, TABDURSYM, STEM}))
            return NULL;
        ...
        if (beamParent != NULL)
        {
            ...
            if (this->Is(STEM))
            {
                ...
                if (!graceElement)
                    graceElement = vrv_cast<const LayerElement *>(this->GetFirstAncestor(CHORD));
                assert(graceElement);
            }
    ...
    const Staff *LayerElement::GetAncestorStaff(const StaffSearch strategy, const bool assertExistence) const
    {
        ...
        if (!staff)
            staff = vrv_cast<const Staff *>(this->GetFirstAncestor(STAFF));
        if (assertExistence)
            assert(staff);
        return staff;
    }
    ...
    const Staff *LayerElement::GetCrossStaff(const Layer *&layer) const
    {
        ...
        const LayerElement *parent = dynamic_cast<const LayerElement *>(this->GetFirstAncestorInRange(LAYER_ELEMENT, LAYER_ELEMENT_max));

        if (parent)
            return parent->GetCrossStaff(layer);
    ...
    int LayerElement::GetDrawingX() const
    {
        ...
        if (m_xAbs != VRV_UNSET)
            return m_xAbs + this->GetDrawingXRel();

        if (m_cachedDrawingX != VRV_UNSET)
    ...
    int LayerElement::GetDrawingY() const
    {
        ...
        if (!object && !this->IsRelativeToStaff())
            object = this->GetFirstAncestorInRange(LAYER_ELEMENT, LAYER_ELEMENT_max);
        // Otherwise get the first staff
        if (!object)
            object = this->GetFirstAncestor(STAFF);
        // Otherwise the first measure (this is the case with barLineAttr)
        if (!object)
            object = this->GetFirstAncestor(MEASURE);
    ...
    int LayerElement::GetDrawingArticulationTopOrBottom(data_STAFFREL place, ArticType type) const
    {
        ...
        for (auto &child : artics)
        {
            ...
            assert(artic);
            if (artic->GetDrawingPlace() == place)
                break;
        ...
        const Mensur *mensur, const MeterSig *meterSig, bool notGraceOnly, data_NOTATIONTYPE notationType) const
    {
        ...
        else if (this->Is({HALFMRPT, MREST, MULTIREST, MRPT, MRPT2, MULTIRPT}))
        {
            ...
            if (meterSig && meterSig->HasCount())
                meterCount = meterSig->GetTotalCount();
    ...
    {
        if (firstChord.empty() || secondChord.empty())
            return {};
        ...
        if (!difference.empty())
        {
            ...
            {
                if (((firstChord.size() <= secondChord.size()) && (element > *firstChord.begin()) && (element < *firstChord.rbegin())) || ((firstChord.size() > secondChord.size()) && (element > *secondChord.begin()) && (element < *secondChord.rbegin())))
                {
                    return {};
                }
            }
        ...
        {
            if ((*firstChord.rbegin() > *secondChord.rbegin()) || (*firstChord.begin() > *secondChord.begin()))
                return {};
        }
        else
        {
    ...
    int LayerElement::CalcLayerOverlap(const Doc *doc, int direction, int y1, int y2)
    {
        ...
        for (auto object : collidingElementsList)
        {
            ...
            if (!this->HorizontalContentOverlap(object))
                continue;
            if (direction > 0)
            {
                // make sure that there's actual overlap first
                if ((layerElement->GetDrawingBottom(doc, staff->m_drawingStaffSize, true) > y1) && (layerElement->GetDrawingBottom(doc, staff->m_drawingStaffSize, true) > y2))
                    continue;
            ...
            {
                // make sure that there's actual overlap first
                if ((layerElement->GetDrawingTop(doc, staff->m_drawingStaffSize, true) < y1) && (layerElement->GetDrawingTop(doc, staff->m_drawingStaffSize, true) < y2))
                    continue;
    ...
    wchar_t LayerElement::StemModToGlyph(data_STEMMODIFIER stemMod) const
    {
        ...
        case STEMMODIFIER_1slash:
            return SMUFL_E220_tremolo1;
        case STEMMODIFIER_2slash:
            return SMUFL_E221_tremolo2;
        case STEMMODIFIER_3slash:
            return SMUFL_E222_tremolo3;
        case STEMMODIFIER_4slash:
            return SMUFL_E223_tremolo4;
        case STEMMODIFIER_5slash:
            return SMUFL_E224_tremolo5;
        case STEMMODIFIER_6slash:
            return SMUFL_E224_tremolo5;
        case STEMMODIFIER_sprech:
            return SMUFL_E645_vocalSprechgesang;
        case STEMMODIFIER_z:
            return SMUFL_E22A_buzzRoll;
        default:
            break;
        }
        return 0;
    ...
    int LayerElement::GetDotCount(const MapOfDotLocs &dotLocations)
    {
        return std::accumulate(dotLocations.cbegin(), dotLocations.cend(), 0,
                               [](int sum, const MapOfDotLocs::value_type &mapEntry)
                               { return sum + (int)mapEntry.second.size(); });
    }
    ...
    std::pair<int, int> LayerElement::CalculateXPosOffset(FunctorParams *functorParams)
    {
        ...
        bool performBoundingBoxAlignment = (params->m_previousAlignment.m_alignment && params->m_previousAlignment.m_alignment->PerformBoundingBoxAlignment() && this->GetAlignment()->PerformBoundingBoxAlignment());
        ...
        for (const auto &boundingBox : params->m_boundingBoxes)
        {
            ...
            else if (this->Is(ACCID) && element->Is(NOTE))
            {
                ...
                if ((this->GetContentTop() > staffTop + 2 * drawingUnit) && (element->GetDrawingY() > staffTop) && (element->GetDrawingY() > this->GetDrawingY()))
                {
                    verticalMargin = element->GetDrawingY() - this->GetDrawingY();
                }
                else if ((this->GetContentBottom() < staffBottom - 2 * drawingUnit) && (element->GetDrawingY() < staffBottom) && (element->GetDrawingY() < this->GetDrawingY()))
                {
                    verticalMargin = this->GetDrawingY() - element->GetDrawingY();
                }
                overlap = std::max(overlap, boundingBox->HorizontalRightOverlap(this, params->m_doc, margin, verticalMargin));
            }
            ...
            if (!overlap)
            {
                ...
                if (this->Is({NOTE, CHORD}) && !this->GetFirstAncestor(TUPLET) && element->Is(REST) && element->GetFirstAncestor(TUPLET))
                {
                    Rest *rest = vrv_cast<Rest *>(element);
    ...
    int LayerElement::ApplyPPUFactor(FunctorParams *functorParams)
    {
        ...
        if (m_xAbs != VRV_UNSET)
            m_xAbs /= params->m_page->GetPPUFactor();
    ...
    int LayerElement::AlignHorizontally(FunctorParams *functorParams)
    {
        ...
        {
            if (m_alignment->AddLayerElementRef(this))
                params->m_hasMultipleLayer = true;
        }
    ...
    int LayerElement::CalcAlignmentPitchPos(FunctorParams *functorParams)
    {
        ...
        else if (this->Is(REST))
        {
            ...
            else
            {
                ...
                Staff *staff = this->GetAncestorStaff();
                loc = staff->m_drawingLines - 1;
                if ((rest->GetDur() < DUR_4) && (loc % 2 != 0))
                    --loc;
                // Adjust special cases
                if ((rest->GetDur() == DUR_1) && (staff->m_drawingLines > 1))
                    loc += 2;
                if ((rest->GetDur() == DUR_BR) && (staff->m_drawingLines < 2))
                    loc -= 2;
    ...
    int LayerElement::AdjustBeams(FunctorParams *functorParams)
    {
        ...
        if (!params->m_beam)
            return FUNCTOR_CONTINUE;
        if (!params->m_isOtherLayer && !this->Is(ACCID) && !this->IsGraceNote() && ((this->GetFirstAncestor(BEAM) == params->m_beam) || (this->GetFirstAncestor(FTREM) == params->m_beam)))
            return FUNCTOR_CONTINUE;
        // ignore elements that are both on other layer and cross-staff
        if (params->m_isOtherLayer && m_crossStaff)
            return FUNCTOR_CONTINUE;
        // ignore specific elements, since they should not be influencing beam positioning
        if (this->Is({BTREM, GRACEGRP, SPACE, TUPLET, TUPLET_BRACKET, TUPLET_NUM}))
            return FUNCTOR_CONTINUE;
        // ignore elements that start before the beam
        if (this->GetDrawingX() < params->m_x1)
            return FUNCTOR_CONTINUE;
        ...
        if (params->m_directionBias > 0)
        {
            ...
            rightMargin = this->GetContentTop() - currentBeamYRight + beamCount * beam->m_beamWidth + beam->m_beamWidthBlack;
        }
        else
        {
            leftMargin = this->GetContentBottom() - currentBeamYLeft - beamCount * beam->m_beamWidth - beam->m_beamWidthBlack;
            rightMargin = this->GetContentBottom() - currentBeamYRight - beamCount * beam->m_beamWidth - beam->m_beamWidthBlack;
        }
        ...
        if (overlapMargin >= params->m_directionBias * params->m_overlapMargin)
        {
            ...
            params->m_overlapMargin = (((overlapMargin + staffOffset - 1) / staffOffset + 0.5) * staffOffset) * params->m_directionBias;
        }
                                              ...
                                              bool areDotsAdjusted, bool &isUnison, bool &stemSameas)
    {
        ...
        {
            LayerElement *parent = vrv_cast<LayerElement *>(this->GetFirstAncestorInRange(LAYER_ELEMENT, LAYER_ELEMENT_max));
            assert(parent);
                                                                    ...
                                                                    bool unison)
    {
        ...
        for (int i = 0; i < int(otherElements.size()); ++i)
        {
            ...
            else if (this->Is(NOTE) && otherElements.at(i)->Is(NOTE))
            {
                ...
                if (unison && currentNote->IsUnisonWith(previousNote, false))
                {
                    ...
                    if (isPreviousCoord)
                    {
                        ...
                        isEdgeElement = ((STEMDIRECTION_down == stemDir) && (parentChord->GetBottomNote() == previousNote)) || ((STEMDIRECTION_up == stemDir) && (parentChord->GetTopNote() == previousNote));
                    }
                    ...
                    if (!isPreviousCoord || isEdgeElement || isChordElement)
                    {
                        ...
                        else if ((!currentNote->IsGraceNote() && !currentNote->GetDrawingCueSize()) && (previousNote->IsGraceNote() || previousNote->GetDrawingCueSize()) && (STEMDIRECTION_down == stemDir))
                        {
                            shift -= 0.8 * horizontalMargin;
                            continue;
                        }
                        else if ((currentNote->IsGraceNote() || currentNote->GetDrawingCueSize()) && (!previousNote->IsGraceNote() && !previousNote->GetDrawingCueSize()) && (STEMDIRECTION_up == stemDir))
                        {
                ...
                else if ((previousNote->GetDrawingLoc() - currentNote->GetDrawingLoc() < 0) && (previousNote->GetDrawingStemDir() != currentNote->GetDrawingStemDir()) /* && !isChordElement*/)
                {
            ...
            else if (this->Is(DOTS) && !otherElements.at(i)->Is(DOTS) && areDotsAdjusted)
            {
                ...
                if (dot->IsAdjusted() || !HorizontalSelfOverlap(otherElements.at(i), horizontalMargin))
                    continue;
            ...
            else if (this->Is(ACCID) && otherElements.at(i)->Is(NOTE))
            {
                ...
                const bool isUnisonOverlap = parentNote && parentNote->IsUnisonWith(otherNote, true) && !parentNote->IsUnisonWith(otherNote, false);
            ...
            if (this->Is(NOTE) && !otherElements.at(i)->Is(STEM))
            {
                ...
                if (!VerticalSelfOverlap(otherElements.at(i), verticalMargin))
                    continue;
                ...
                else
                {
                    ...
                    shift -= horizontalMargin - HorizontalRightOverlap(otherElements.at(i), doc, horizontalMargin - shift, verticalMargin);
                }
            }
            else if (this->Is(NOTE))
            {
                ...
                if (otherElements.at(i)->Is(STEM) && (shift == 0) && areDotsAdjusted)
                {
                    ...
                    if (currentNote->HasStemSameasNote())
                        continue;
        ...
        {
            Note *currentNote = vrv_cast<Note *>(this);
            if (currentNote->GetDrawingAccid())
                shift *= -1;
        }
    ...
    int LayerElement::AdjustGraceXPos(FunctorParams *functorParams)
    {
        ...
        int selfLeft = this->GetSelfLeft() - params->m_doc->GetLeftMargin(this) * params->m_doc->GetDrawingUnit(params->m_doc->GetCueSize(100));
        ...
        if (it != params->m_measureTieEndpoints.end() && params->m_rightDefaultAlignment)
        {
            ...
            if (diff < (minTieLength + unit))
                params->m_graceMaxPos -= (unit + minTieLength - diff);
        }
    ...
    int LayerElement::AdjustTupletNumOverlap(FunctorParams *functorParams) const
    {
        ...
        if (!this->Is({ARTIC, ACCID, CHORD, DOT, FLAG, NOTE, REST, STEM}) || !this->HasSelfBB())
            return FUNCTOR_CONTINUE;

        if (this->Is({CHORD, NOTE, REST}) && ((m_crossStaff || (this->GetFirstAncestor(STAFF) != params->m_staff)) && (m_crossStaff != params->m_staff)))
            return FUNCTOR_SIBLINGS;

        if (!params->m_tupletNum->HorizontalSelfOverlap(this, params->m_horizontalMargin) && !params->m_tupletNum->VerticalSelfOverlap(this, params->m_verticalMargin))
        {
            return FUNCTOR_CONTINUE;
        }
    ...
    int LayerElement::AdjustXPos(FunctorParams *functorParams)
    {
        ...
        Alignment *nextAlignment = vrv_cast<Alignment *>(this->GetAlignment()->GetParent()->GetNext(this->GetAlignment(), ALIGNMENT));
        ...
        if (it != params->m_measureTieEndpoints.end())
        {
            ...
            if ((currentTieLength < minTieLength) && ((it->first->GetFirstAncestor(CHORD) != NULL) || (this->GetFirstAncestor(CHORD) != NULL) || (it->first->FindDescendantByType(FLAG) != NULL)))
            {
    ...
    int LayerElement::PrepareCueSize(FunctorParams *functorParams)
    {
        ...
        else if (this->HasAttClass(ATT_CUE))
        {
            ...
            assert(att);
            if (att->HasCue())
                m_drawingCueSize = (att->GetCue() == BOOLEAN_true);
        }
        ...
        else if (this->Is(NOTE))
        {
            ...
            Chord *chord = note->IsChordTone();
            if (chord)
                m_drawingCueSize = chord->GetDrawingCueSize();
        }
        ...
        else if (this->Is(TUPLET))
        {
            ...
            if (child)
                m_drawingCueSize = child->GetDrawingCueSize();
        }
    ...
    int LayerElement::PrepareTimePointing(FunctorParams *functorParams)
    {
        ...
        if (this->Is({ARTIC, BEAM, FLAG, TUPLET, STEM, VERSE}))
            return FUNCTOR_CONTINUE;
    ...
    int LayerElement::LayerElementsInTimeSpan(FunctorParams *functorParams) const
    {
        ...
        if ((!params->m_allLayersButCurrent && (currentLayer != params->m_layer)) || (params->m_allLayersButCurrent && (currentLayer == params->m_layer)))
        {
            return FUNCTOR_SIBLINGS;
        }
        if (!currentLayer || this->IsScoreDefElement() || this->Is(MREST))
            return FUNCTOR_SIBLINGS;
        if (!this->GetDurationInterface() || this->Is({MSPACE, SPACE}) || this->HasSameasLink())
            return FUNCTOR_CONTINUE;
        ...
        if ((time + duration) <= params->m_time)
            return FUNCTOR_CONTINUE;
        // The element is starting after the event end - we can stop here
        if (time >= (params->m_time + params->m_duration))
            return FUNCTOR_STOP;
    ...
    int LayerElement::FindSpannedLayerElements(FunctorParams *functorParams) const
    {
        ...
        if (this->HasContentBB() && !this->HasEmptyBB() && (this->GetContentRight() > params->m_minPos) && (this->GetContentLeft() < params->m_maxPos))
        {
    ...
    int LayerElement::GetRelativeLayerElement(FunctorParams *functorParams) const
    {
        ...
        if (this->Is(REST))
            return params->m_isInNeighboringLayer ? FUNCTOR_STOP : FUNCTOR_SIBLINGS;
